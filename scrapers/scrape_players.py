...
class PlayerScraper(Scraper):
    ...
    def clean_table(self, table: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        #     result['playing_time_full_90s'] = result['playing_time_full_90s'].fillna(value=0.0)
        #
        # if 'performance_gls' in result.columns:
        #     result['performance_gls'] = result['performance_gls'].fillna(value=0)
        #
        # if 'standard_sh' in result.columns:
        #     result['standard_sh'] = result['standard_sh'].fillna(value=0)
        #
        # if 'standard_fk' in result.columns:
        #     result['standard_fk'] = result['standard_fk'].fillna(value=0)
        #
        # if 'total_att' in result.columns:
        #     result['total_att'] = result['total_att'].fillna(value=0)
        #
